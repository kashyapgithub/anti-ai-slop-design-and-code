...
# (anti-ai-slop-code.md §18: format, type-check, lint, unit tests,
...
# Run those explicitly with the commands in §18, or wire
...
if [[ "$failed" -eq 1 ]]; then
  ...
  echo "layers after this one weren't run, per §18: a change that fails an"
  echo "early layer produces noise, not signal, at the later ones."
  exit 1
fi

echo
echo "All configured mechanical layers passed. Remember: layers 4/5, 8, 9,"
echo "and 10 from §18 still need to happen — this script only covers what"
