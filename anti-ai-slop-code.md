...
1. **Create `ARCHITECTURE.md` (or `AGENTS.md`, matching whatever convention file your tooling reads automatically) as close to the first message as the idea is concrete enough to support it.** Before writing it, ask — don't guess the shape of the thing from a one-line request. A short, structured set of questions here is what actually makes the architecture worth writing down, instead of a plausible-sounding guess that gets rewritten twice in the first week.
   ...
   **Advanced — ask only the ones the basic answers actually make relevant, not all of them reflexively:**
   ...
   - If it stores data: what kind, what the actual access patterns look like (mostly reads or mostly writes, lookups by key or complex queries across relationships, does old data need to stay queryable or can it age out), and whether it needs strong consistency/transactions or eventual consistency is acceptable. This is the question that actually determines data structure and storage choice — not "SQL or NoSQL" as an abstract preference, but what the real read/write shape requires.
   ...
   **Propose the specific data structures and technologies that fit the answers above — and just as importantly, say plainly which common ones you're *not* recommending and why.** This is not optional politeness; it's the actual point of asking first. Two concrete moves, every time:
   - **Name the real data structure the access pattern implies**, not a generic "a database." "Reads dwarf writes and lookups are always by user ID" points at a simple key-value or indexed-table lookup; "needs full-text search across descriptions" points at a search index, not a `LIKE '%...%'` query; "needs to know what changed and when" points at an append-only log or event table, not overwriting rows in place. Say which one fits and why, in terms of the actual access pattern just described — not a default reached for out of habit.
   - **When a heavier piece of infrastructure comes up — Redis, Docker, a message queue, microservices, Kubernetes — state the case against it for this project as clearly as the case for it**, and default to the simpler option unless the basic/advanced answers actually justify the heavier one. "Redis" is not the answer to "we need to cache something" when the app has one process and a few hundred users — an in-process cache or the database's own query cache does the same job with one less moving part to operate, monitor, and explain to the next person. "Microservices" is not the answer to "this might need to scale someday" when there's one team and one deploy target — a well-factored monolith (§17's feature-first structure already gives you the seams to split later, if it's ever actually needed) gets you there without the operational tax paid up front. Docker is worth it the moment there's a real deployment or environment-parity problem to solve, and adds nothing when there isn't one yet. Naming the tradeoff explicitly — "here's what X would buy you, here's what it costs, and here's why I don't think this project needs it yet" — is what separates a real recommendation from reflexive over-engineering, and it's the direct, concrete form of the Rule of Three restraint (§17.2) that the rest of this guide argues for in the abstract.
